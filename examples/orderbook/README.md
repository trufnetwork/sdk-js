...
### 2. Place Orders (`02_place_orders.ts`)

Market maker provides two-sided liquidity to earn LP rewards.

```bash
npx tsx examples/orderbook/02_place_orders.ts
```
...
### 3. Take Orders (`03_take_orders.ts`)

Takers execute trades against the market maker's orders.

```bash
npx tsx examples/orderbook/03_take_orders.ts
```
...
### 4. Verify State (`04_verify_state.ts`)

Queries the order book state using SDK methods to verify results.

```bash
npx tsx examples/orderbook/04_verify_state.ts
```
...
### "Market not found"
Run `01_create_market.ts` first to create a market and save the query_id.
