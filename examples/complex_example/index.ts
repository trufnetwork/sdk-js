...
async function main() {
	...
	const compositeIndexStreamId = await StreamId.generate(
		"commodity_composite_index"
	);

	try {
		// 1. Deploy Primitive Streams
		const deployTx1 = await client.deployStream(
			priceStreamId1,
		...
		const deployTx2 = await client.deployStream(
			priceStreamId2,
			StreamType.Primitive
		);
		...
		const deployComposedTx = await client.deployStream(
			compositeIndexStreamId,
			StreamType.Composed
		);
		...
		const destroyTx1 = await client.destroyStream(
			client.ownStreamLocator(priceStreamId1)
		);
		if (!destroyTx1.data) throw new Error("Destroy Tx1 failed");
		await client.waitForTx(destroyTx1.data.tx_hash);

		const destroyTx2 = await client.destroyStream(
			client.ownStreamLocator(priceStreamId2)
		);
		if (!destroyTx2.data) throw new Error("Destroy Tx2 failed");
		await client.waitForTx(destroyTx2.data.tx_hash);

		const destroyComposedTx = await client.destroyStream(
			client.ownStreamLocator(compositeIndexStreamId)
		);
